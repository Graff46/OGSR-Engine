...
ID3DBaseTexture* CRender::texture_load(LPCSTR fRName, u32& ret_msize, bool bStaging)
{
    ...
    IReader* S = NULL;
    if ((!FS.exist(fn, "$game_textures$", fname, ".dds")) && (strstr(fname, "_bump")) && (!FS.exist(fn, "$game_textures_ex$",	fname,	".dds")))
        goto _BUMP_from_base;
    if (FS.exist(fn, "$level_textures_ex$", fname, ".dds"))
        goto _DDS;
    if (FS.exist(fn, "$level$", fname, ".dds"))
        goto _DDS;
    if (FS.exist(fn, "$game_saves$", fname, ".dds"))
        goto _DDS;
    if (FS.exist(fn, "$game_textures_ex$",	fname, ".dds"))
        goto _DDS;
